...
export const users = pgTable("users", {
  ...
  sdrId: varchar("sdr_id", { length: 50 }).references(() => sdrs.id),
  managerId: varchar("manager_id").references(() => managers.id),
  accountSpecialistId: varchar("account_specialist_id").references(() => accountSpecialists.id),
  createdAt: timestamp("created_at").defaultNow().notNull(),
...
// Account Specialists - Dedicated role for account management
export const accountSpecialists = pgTable("account_specialists", {
  id: varchar("id").primaryKey().default(sql`gen_random_uuid()`),
  name: text("name").notNull(),
  email: text("email").notNull().unique(),
  managerId: varchar("manager_id").references(() => managers.id),
  coverageAreas: text("coverage_areas"), // JSON string of areas/regions
  specializations: text("specializations"), // JSON string of product specializations
  isActive: boolean("is_active").default(true).notNull(),
  createdAt: timestamp("created_at").defaultNow().notNull(),
});
...
export const usersRelations = relations(users, ({ one, many }) => ({
  ...
  accountSpecialist: one(accountSpecialists, {
    fields: [users.accountSpecialistId],
    references: [accountSpecialists.id],
  }),
