...
import { HeroMetric, FunnelStage, LeaderboardRow, ActionItem, type AnomalyData } from "@/components/hero-metric";
import { useDialogModal } from "@/hooks/use-modal-state";
import { WorkQueue, convertMetricsToWorkQueue } from "@/components/work-queue";
import { 
  Users, 
  Phone, 
  TrendingUp, 
...
// AnomalyData type is now imported from hero-metric component
...
// HeroMetric, FunnelStage, LeaderboardRow, and ActionItem are now imported from @/components/hero-metric
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* Work Queue for SDRs - Shows prioritized tasks */}
        {!metrics?.isPrivileged && metrics?.actionItems && (
          <WorkQueue
            items={convertMetricsToWorkQueue(metrics)}
            onCallLead={(leadId, phone) => {
            ...
            onViewLead={(leadId) => {
              const lead = metrics.actionItems.hotLeads.find(l => l.id === leadId) ||
                          metrics.actionItems.leadsWithoutResearch.find(l => l.id === leadId);
              if (lead) {
                setSelectedLead({
                  id: lead.id,
                  companyName: lead.companyName,
